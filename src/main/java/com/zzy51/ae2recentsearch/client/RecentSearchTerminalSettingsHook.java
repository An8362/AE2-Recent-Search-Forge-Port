...
@Mod.EventBusSubscriber(modid = AE2RecentSearch.MOD_ID, value = Dist.CLIENT)
public final class RecentSearchTerminalSettingsHook {
    ...
    public static void addRecentSearchSettingsTab(ScreenEvent.Init.Post event) {
        ...
        if (!(settings instanceof AEBaseScreenAccessor accessor)) {
            return;
        }

        var toolbar = accessor.ae2RecentSearch$getVerticalToolbar();
        ...
        var button = new RecentSearchSettingsButton(ignored ->
                ((AEBaseScreenAccessor) settings)
                        .ae2RecentSearch$invokeSwitchToScreen(new RecentSearchTerminalSettingsScreen(settings)));
        toolbar.add(button);
